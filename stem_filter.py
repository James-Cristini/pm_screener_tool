...
def strip_names(line):
    ...
    return [x.strip(string.whitespace) for x in names_on_line if x.strip(string.whitespace)]

def get_stem_conflicts(names_list, ignore, file_name, check_type):
    # Create dictionary with each line as a key and each value as a list of stem conflicts
    names_dict = {}
    for line in names_list:
        names_dict[line] = find_stems([x.lower() for x in strip_names(line)], ignore, check_type)

    build_doc(names_list, names_dict, file_name)

def find_stems(names_list, ignore, check_type):
    ...
    for name in names_list:
        for infix in INN_STEMS["Infix"]:
            ...
            if infix in name and infix not in ignore:
                ...
                if (" (" + defin + ")") not in  " ".join(conflicts):
                    if check_type == "PHARMA" and "USAN" not in defin:
